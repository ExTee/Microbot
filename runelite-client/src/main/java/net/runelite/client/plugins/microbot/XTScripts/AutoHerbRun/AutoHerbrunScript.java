...
@Slf4j
public class AutoHerbrunScript extends Script {
    @Inject
    private ConfigManager configManager;
    @Inject
    private FarmingWorld farmingWorld;
    private FarmingHandler farmingHandler;
    private final AutoHerbrunPlugin plugin;
    private final AutoHerbrunConfig config;
    ...
    @Inject
    public AutoHerbrunScript(AutoHerbrunPlugin plugin, AutoHerbrunConfig config) {
        this.plugin = plugin;
        this.config = config;
    }
    ...
    public boolean run() {        
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            ...
            if (!initialized) {
                initialized = true;
                AutoHerbrunPlugin.status = "Gearing up";
                populateHerbPatches();
...
//                        Rs2Bank.withdrawAndEquip(13280);
            ...
            if (currentPatch == null) {
                AutoHerbrunPlugin.status = "Finishing up";
                if (config.goToBank()) {
                    Rs2Walker.walkTo(Rs2Bank.getNearestBank().getWorldPoint());
                    if (!Rs2Bank.isOpen()) Rs2Bank.openBank();
                    Rs2Bank.depositAll();
                }
                AutoHerbrunPlugin.status = "Finished";
            ...
            if (!currentPatch.isInRange(20)) {
                ...
                switch (currentPatch.getRegionName()){
                    case "Harmony":
                        goHomeAndTeleportNexus('y');
                        break;
                    case "Weiss":
                        goHomeAndTeleportNexus('w');
                        break;
                    case "Troll Stronghold":
                        goHomeAndTeleportNexus('t');
                        break;
                    case "Catherby":
                        goHomeAndTeleportNexus('c');
                        break;
                }
...
//                }
//
//                if (currentPatch.getRegionName().equalsIgnoreCase("Weiss")){
//
//                    Rs2Inventory.interact("Construct. cape(t)", "Tele to POH");
////                    Rs2Equipment.interact("Max cape", "Home");
//                    sleepUntil(() -> Rs2GameObject.exists(33410));
//                    Rs2GameObject.interact("Portal Nexus", "Teleport Menu");
//                    sleepUntil(() -> (Rs2Widget.getWidget(1114113) != null));
//                    Rs2Keyboard.keyPress('w');
//                    sleep(1000);
//
//                }

//                if (currentPatch.getRegionName().equalsIgnoreCase("Troll Stronghold")){
//
//                    Rs2Inventory.interact("Construct. cape(t)", "Tele to POH");
////                    Rs2Equipment.interact("Max cape", "Home");
//                    sleepUntil(() -> Rs2GameObject.exists(33410));
//                    Rs2GameObject.interact("Portal Nexus", "Teleport Menu");
//                    sleepUntil(() -> (Rs2Widget.getWidget(1114113) != null));
//                    Rs2Keyboard.keyPress('t');
//                    sleep(1000);
//
//                }

//                if (currentPatch.getRegionName().equalsIgnoreCase("Catherby")){
//
//                    Rs2Inventory.interact("Construct. cape(t)", "Tele to POH");
////                    Rs2Equipment.interact("Max cape", "Home");
//                    sleepUntil(() -> Rs2GameObject.exists(33410));
...
//                if (currentPatch.getRegionName().equalsIgnoreCase("Farming Guild")){
//
////                    Rs2Inventory.interact("Construct. cape(t)", "Tele to POH");
//                    Rs2Equipment.interact("Max cape", "Home");
//                    sleepUntil(() -> Rs2GameObject.exists(33410));
//                    Rs2GameObject.interact("Ornate Jewellery Box", "Teleport Menu");
//                    sleepUntil(() -> (Rs2Widget.getWidget(38666240) != null));
//                    Rs2Keyboard.keyPress('j');
//                    sleep(1000);
//
//                }
...
//                if (currentPatch.getRegionName().equalsIgnoreCase("Ardougne")){
//
//                    Rs2Inventory.interact("Ardougne cloak 4", "Farm Teleport");
//                    sleep(1000);
//
//                }
//
//                if (currentPatch.getRegionName().equalsIgnoreCase("Morytania")){
//
//                    Rs2Inventory.interact("Ectophial", "Empty");
//                    sleep(1000);
//
//                }
//
//                if (currentPatch.getRegionName().equalsIgnoreCase("Falador")){
//
//                    Rs2Inventory.interact("Explorer's ring 4", "Teleport");
//                    sleep(1000);
//
//                }
//
//                if (currentPatch.getRegionName().equalsIgnoreCase("Kourend")){
//
//                    Rs2Inventory.interact("Xeric's talisman", "Xeric's Glade");
//                    sleep(1000);
//
//                }
//
//                if (currentPatch.getRegionName().equalsIgnoreCase("Varlamore")){
//
//                    Rs2Inventory.interact("Perfected quetzal whistle", "Signal");
//                    sleep(1000);
//
//                }
                ...
                AutoHerbrunPlugin.status = "Walking to " + currentPatch.getRegionName();
                Rs2Walker.walkTo(currentPatch.getLocation(), 30);

            }

            AutoHerbrunPlugin.status = "Farming " + currentPatch.getRegionName();
    ...
    private void goHomeAndTeleportNexus(char hotkey){
        final String CAPE = "Construct. cape(t)";
        final String CAPE_ACTION = "Tele to POH";
        final int PORTAL_NEXUS_ID = 33410;
        final int TELEPORT_MENU_WIDGET_ID = 1114113;

        // Go Home
        Rs2Inventory.interact(CAPE, CAPE_ACTION);
        sleepUntil(() -> Rs2GameObject.exists(PORTAL_NEXUS_ID));
        Rs2GameObject.interact("Portal Nexus", "Teleport Menu");
        sleepUntil(() -> (Rs2Widget.getWidget(1114113) != null));
        ...
        switch (state) {
            case "Empty":
                ...
                if (config.compostType() != CompostType.NONE) {
                    ...
                    Rs2Player.waitForXpDrop(Skill.FARMING, 15000, false);
            ...
            case "Harvestable":
                ...
                Rs2Player.waitForXpDrop(Skill.FARMING, 35000, false);
                Rs2GameObject.interact(obj, "Pick");
                sleep(200);
                Rs2GameObject.interact(obj, "Pick");
                sleep(200);
